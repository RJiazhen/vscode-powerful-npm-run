 ...
 */
export async function getNpmScriptFromPackageJson(
  ...
  const packageJsonTextDocument =
    await vscode.workspace.openTextDocument(packageJsonPath);
  ...
  const packageScriptList = Object.entries(packageJsonScripts).map(
    ([name, command]) => ({
      name,
